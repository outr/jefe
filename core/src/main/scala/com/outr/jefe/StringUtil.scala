...
object StringUtil {
  /**
    ...
    */
  def toCamelCase(name: String): String = "[- _]([a-zA-Z0-9])".r.replaceAllIn(name, m => m.group(1).toUpperCase)

  /**
    * Converts camelCase to dash-separated.
    */
  def fromCamelCase(name: String): String = "([A-Z])".r.replaceAllIn(name, m => "-" + m.group(0).toLowerCase)
  ...
  def trim(s: String): String = s match {
